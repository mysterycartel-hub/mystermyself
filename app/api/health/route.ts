...
async function testBeehiiv(): Promise<{
  ...
  if (!apiKey || apiKey.length < 20) {
    result.error = 'Missing or too short BEEHIIV_API_KEY'
    return result
  }
...
async function testStripe(): Promise<{
  ...
  if (!secretKey || !secretKey.startsWith('sk_')) {
    result.error = 'Missing or invalid STRIPE_SECRET_KEY — must start with sk_live_ or sk_test_'
    return result
  }
...
export async function GET() {
  ...
  const allConnected = supabase.connected && beehiiv.connected && stripe.connected
  const someConnected = supabase.connected || beehiiv.connected || stripe.connected

  const status = allConnected ? 'healthy' : someConnected ? 'degraded' : 'broken'
