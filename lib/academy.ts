...
export const CHARACTERS: Record<string, Character> = {
  ...
  'melissa-mayhem': {
    id: 'melissa-mayhem',
    name: 'Melissa Mayhem',
    title: 'Fakeout & Chop Specialist',
    emoji: '⚡',
    color: '#EC4899',
    role: 'warning',
    catchphrase: "I am everywhere in this market. Stop chasing me. You will never catch me.",
    teaches: ['wicks', 'flow'],
  },
  'melody-mayhem': {
    id: 'melody-mayhem',
    name: 'Melody Mayhem',
    title: 'Emotions & Trade Management',
    emoji: '🎭',
    color: '#F59E0B',
    role: 'warning',
    catchphrase: "Your emotions are my favorite trading partner. They always make the wrong call at the perfect time.",
    teaches: ['risk', 'management'],
  },
  'burn-alarm': {
    id: 'burn-alarm',
    ...
    color: '#EF4444',
    role: 'alert',
    catchphrase: "STOP. Check your Burn Point before anything else.",
    teaches: ['risk'],
  },
}
...
export interface Lesson {
  ...
  hook: string                    // 1. Why this matters
  kitchenStory: string            // 2. Food/kitchen metaphor
  marketTranslation: string       // 3. Actual trading concept
  visualGuide: string             // 4. What to look for on chart
  characterCoaching: string       // 5. Character's teaching moment
  practice: PracticeTask          // 6. One-thing practice
  xpReward: number                // 8. XP on completion
  journalPrompts: JournalPrompt[] // 9. Reflection
  psychologyNote?: string         // Trading psychology insert
  riskWarning?: string            // Burn Alarm trigger
...
export const LESSONS: Lesson[] = [
  // ── MODULE 1: CANDLES ────────────────────────────────────────
  {
    ...
    practice: {
      ...
      hint: "A bullish candle has its close above its open. It is usually shown in green or white. A bearish candle has its close below its open — red or black.",
  ...
  // ── MODULE 2: WICKS ──────────────────────────────────────────
  {
    ...
    visualGuide: "On any chart, zoom out to see 20+ candles. Look for candles with wicks that are at least twice the size of the body. These are rejection wicks. Now ask: are they rejecting from a level that has been tested before? Repeated rejection from the same level = strong signal.",
    ...
    practice: {
      ...
      hint: "Long wicks at the top of a move often signal a reversal downward. Long wicks at the bottom often signal a reversal upward. Look for wicks that stand out from the surrounding candles.",
    ...
    psychologyNote: "Revenge trading often starts with a wick. Price hits your stop, reverses, and you feel cheated. Melissa Mayhem wins when you revenge trade. The proper response is to note the wick in your journal, wait for the next setup, and never add to a losing position.",
    nextLesson: 'bias',
    prevLesson: 'candles',
  },
  ...
  // ── MODULE 4: FLOW ───────────────────────────────────────────
  {
    ...
    color: '#3B82F6',
    icon: '🌊',
    hook: "Markets do not move randomly. They move with purpose — toward liquidity. Old highs, old lows, equal levels, Asian range extremes. Once you see liquidity pools, you cannot unsee them. And once you track the flow, you move with the smart money instead of against it.",
    ...
    psychologyNote: "Chasing candles is how traders lose. When you see price sweeping a liquidity pool, the instinct is to jump in immediately. This is Melody Mayhem in action. The proper move: mark the pool in advance, set a limit order, and walk away. Either it fills or it does not. No chasing.",
  ...
  // ── MODULE 5: STRUCTURE ──────────────────────────────────────
  {
    ...
    marketTranslation: "In an uptrend: price makes higher highs (HH) and higher lows (HL). A Break of Structure (BOS) to the upside confirms the trend continues. A Change of Character (CHOCH) — a break below the most recent higher low — signals potential reversal. Ranges form when price stops making HH/HL or LL/LH and instead bounces between two levels. Expansion is when price breaks from a range with momentum.",
    visualGuide: "On XAUUSD Daily, identify the most recent swing high and swing low. Label them. Then identify the one before each. Are the highs getting higher? Are the lows getting higher? If yes — bullish structure. If the most recent low breaks below the previous low — that is a CHOCH. The structure has changed.",
    characterCoaching: "Chef Goldie says: 'I do not guess which way a market is going. I read what it already told me. Three higher lows in a row with three higher highs — that is the market speaking bullish. A single lower low does not erase that conversation. But two lower lows with a lower high in between? Now the recipe has changed. I close my bullish position and wait.'",
    practice: {
      instruction: "On XAUUSD Daily, identify: (1) The most recent BOS — the candle that broke above a prior swing high (if bullish) or below a prior swing low (if bearish). (2) The most recent CHOCH — the first time a key structure level broke in the opposing direction. Write the approximate price and date of each.",
  ...
  // ── MODULE 6: AOI ────────────────────────────────────────────
  {
    ...
    color: '#F97316',
    icon: '🏡',
    hook: "The most common mistake new traders make is buying in the wrong zone. Nana Value has one rule she has lived by for forty years: never overpay. In trading, the zone you enter from determines everything — your risk, your reward, your probability. Get this right and the rest is discipline.",
    ...
    journalPrompts: [
      ...
      { question: "Was it difficult to mark a zone and then wait? The urge to chase is Melody Mayhem. Write down what you will do instead of chasing if price moves without you.", type: 'emotion' },
    ],
  ...
  // ── MODULE 7: RISK ───────────────────────────────────────────
  {
    ...
    color: '#c0392b',
    icon: '🔔',
    hook: "Risk management is not optional. It is the only reason any trader survives long enough to become profitable. Grandma Market has been trading since before most students were born. She has seen every mistake. And she will tell you: the traders who blow accounts are almost never wrong about direction. They are wrong about size.",
    ...
    marketTranslation: "The Burn Point (stop loss) is placed at the level that, if hit, proves the trade was wrong. Not just uncomfortable — wrong. For a bullish trade from an AOI, the Burn Point sits below the AOI. For a bearish trade, above. Risk no more than 1-2% of account balance on any single pass. This means your position size is calculated backward from your Burn Point — not from how many contracts feel right.",
    ...
    journalPrompts: [
      ...
      { question: "If you hit 5 consecutive losses at 1% risk — how much of your account remains? (Answer: ~95%.) How does that change your psychology versus hitting 5 losses at 5% risk?", type: 'invalidation' },
    ...
    psychologyNote: "Moving the stop loss is the single most dangerous habit in trading. It is Melody Mayhem's favorite trick. The moment your finger hovers over that stop — close the platform. Walk away. Come back in 15 minutes. The urge will pass. The trade will resolve. And your account will thank you.",
  ...
  // ── MODULE 8: DELIVERY ───────────────────────────────────────
  {
    ...
    marketTranslation: "Impulsive delivery: strong, consecutive candles closing near their highs (bullish) or lows (bearish), with momentum and purpose. This shows institutional intent. Corrective delivery: overlapping, choppy candles that barely move net-net, covering their own range. This is either accumulation or distribution — wait, do not trade. Session opens (The Kitchen Is Open) produce the cleanest impulsive delivery.",
    visualGuide: "On XAUUSD 1H, look at the last major move to a key level. Count the candles involved. Are most of them closing in the direction of the move? Are the bodies large relative to wicks? Or are the candles small-bodied, alternating direction, overlapping? The first is impulsive. The second is corrective.",
    ...
    practice: {
      instruction: "On XAUUSD 1H, find one example of impulsive delivery (3+ consecutive candles closing strongly in one direction with minimal overlap) and one example of corrective delivery (choppy, overlapping candles). Screenshot or note the time/price range of each.",
  ...
  // ── MODULE 10: THE PASS ──────────────────────────────────────
  {
    ...
    xpReward: 150,
    journalPrompts: [
      { question: "Write your full trade plan in this format: Symbol | Bias | Entry (The Pass) | Burn Point | Tables Served 1 | Tables Served 2 | Risk %", type: 'observation' },
  ...
  // ── MODULE 11: TABLES SERVED ─────────────────────────────────
  {
    ...
    color: '#3B82F6',
    icon: '🍽️',
    hook: "Your targets are not random price levels. They are the next liquidity pools in the direction of your trade — the places where price is flowing. Louie Liquidity mapped them for you in Module 4. Now you use that map to decide where the profit gets locked in.",
    ...
    visualGuide: "After marking your entry, scroll right on the chart (do not look at price right-to-left, look left-to-right as price moves). Identify: (1) The nearest old high or equal high cluster above price (bullish). That is First Tables Served. (2) The next old high cluster beyond that. Second Tables Served. Mark both levels before placing the trade.",
  ...
  // ── MODULE 12: MANAGEMENT ────────────────────────────────────
  {
    ...
    journalPrompts: [
      ...
      { question: "After a trade — whether it won or lost — what emotion are you feeling? Write it honestly. The journal is private. Patterns in your emotions are the most important data you will collect.", type: 'emotion' },
    ],
    ...
    riskWarning: "BURN ALARM: Never move your Burn Point against the trade. Only move it in the direction of profit (to break even, then to trail). Moving a stop loss against the trade is the fastest way to turn a small loss into a large one.",
    nextLesson: 'kitchen-rush',
    prevLesson: 'tables-served',
  },

  // ── MODULE 13: KITCHEN RUSH (PRACTICE) ──────────────────────
  {
    id: 'kitchen-rush',
    slug: 'kitchen-rush',
    step: 13,
    ...
    color: '#c9a84c',
    icon: '⏱️',
    hook: "You have learned every step of The Recipe. Now it is time to run the kitchen under pressure. The Kitchen Rush is where everything you have learned gets applied in sequence on a live chart. Melissa Mayhem will try to distract you. Chef Goldie will guide you. Your job: run the full Recipe from start to finish without skipping a step.",
    ...
    characterCoaching: "Chef Goldie says: 'Every new trader wants to skip steps. They want to get to The Pass quickly. But the fastest way to a great trade is running the full Recipe slowly. Every step that fails is a trade you saved. Every step that succeeds is a setup worth watching. Run the Recipe. Every time. Without exception.'",
    practice: {
...
export const ACADEMY_MODULES = [
  {
    id: 'module-1',
    name: 'Foundation',
    description: 'Learn to read the language of price before anything else. Candles and wicks are the alphabet of every chart.',
    lessonIds: ['candles', 'wicks'],
  },
  {
    id: 'module-2',
    name: 'The Recipe',
    description: 'The full Trading Chef framework — from reading Bias to placing The Pass. Every step of the setup methodology.',
    lessonIds: ['bias', 'flow', 'structure', 'aoi', 'risk', 'delivery', 'confirmation', 'pass', 'tables-served', 'management'],
  },
  {
    id: 'module-3',
    name: 'Kitchen Rush',
    description: 'Timed simulation practice. Put the full Recipe together under pressure and build real decision speed.',
    lessonIds: ['kitchen-rush'],
  },
]
