...
const defaultMagicData: MagicNode[] = [
  {
    ...
    children: [
      { id: "1-1", name: "Shiny File.txt", type: "file", sparkle: true },
      {
        ...
        name: "Hidden Gems",
        type: "folder",
        children: [{ id: "1-2-1", name: "Gem.js", type: "file", sparkle: true }],
      },
    ],
  },
      ...
      <div key={n.id} className="relative group">
        ...
        >
          {selected === n.id ? (
            ...
            <Folder size={16} className="text-[#c9a84c] shrink-0" />
          ) : (
            <File size={14} className="text-[#94a3b8] shrink-0" />
          )}
          <span className="flex-1 truncate text-sm">{n.name}</span>
          {n.sparkle && n.id !== selected && (
            <span className="text-[10px] text-[#c9a84c] opacity-60">✦</span>
          )}
        </motion.div>
        ...
        <AnimatePresence>
          {n.children && n.children.length > 0 && expanded[n.id] && (
            <motion.div
              ...
              className="pl-4 border-l border-[#c9a84c]/30"
            >
